...
def main() -> int:
    ...
    constraint_acceptance_result = validate_constraint_document_acceptance(repo_root)
    print(
        f"{'PASS' if constraint_acceptance_result.valid else 'FAIL'} "
        f"constraint-document-acceptance errors={list(constraint_acceptance_result.errors)}"
    )
    failures += 0 if constraint_acceptance_result.valid else 1

    assignment_acceptance_result = validate_assignment_document_acceptance(repo_root)
    print(
        f"{'PASS' if assignment_acceptance_result.valid else 'FAIL'} "
        f"assignment-document-acceptance errors={list(assignment_acceptance_result.errors)}"
    )
    ...
    assignment_selection_result = validate_assignment_promotion_selection(repo_root)
    print(
        f"{'PASS' if assignment_selection_result.valid else 'FAIL'} "
        f"assignment-promotion-selection errors={list(assignment_selection_result.errors)}"
    )
