...
- Assignment Canonical-readiness probes apply all seven fixed AD-052 criteria without reusing Accepted-only evidence, rederive the five Canonical direct dependencies and two Canonical Concept dependencies, execute ten current Assignment witnesses and a same-slot `ASSIGNMENT_T6` reachability probe, and preserve the unmet OCP-023 need, eight withheld questions, `0.4.0 / Accepted`, Assignment `Accepted` and `active_cycle_id: null` as independent mutation-live boundaries.
