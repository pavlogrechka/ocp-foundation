...
- Accepted OCP-006 separate Constraint application-order, override and contextual-waiver negative boundaries;
...
- `constraint-interaction-rules.yaml` — Accepted OCP-006 separate application-order, override and contextual-waiver evidence envelopes, negative results and prohibited positive authority.
...
For Accepted OCP-006 `0.4.0`, `constraint-application-order-boundary@1`, `constraint-override-boundary@1` and `constraint-waiver-boundary@1` remain mechanically separate. Exact current Constraint-version inputs bound to one context/snapshot derive only `constraint_application_order_not_established`, `constraint_override_not_established` or `contextual_waiver_not_established`. Application-order replay is permutation and provenance invariant. Missing, ambiguous, stale, cross-bound, cross-branch, self-targeting or positive-coupled evidence fails closed as `indeterminate`; the checker creates no precedence, suppression or exemption authority.
...
- Assignment stable-surface discovery probes independently derive the non-empty Resource/Operation Concept dependency floor and all seven direct consumers, now all Accepted after OCP-006 acceptance, classify all eleven OCP-005 questions and pin six bounded candidates/seven moving surfaces/three remaining blockers. Its pre-selection cycle assertion resolves to byte-exact historical evidence while the live projection now exact-binds active `ASSIGNMENT_T6`.
...
- Constraint document-status readiness probes derive seven lifecycle criteria from exact current OCP-001/OCP-016 tokens, reject promotion criteria invented by stable-surface practice, scan all 25 Accepted/Canonical primary OCP documents and bind the current open-question carriers plus the live unresolved/open lexical surface. They preserve AD-052's Draft subject on its baseline while the live sweep includes current Accepted OCP-005/OCP-006; OCP-006 Canonical L2 still fails because Accepted OCP-005 is below Canonical.
- Constraint document-acceptance probes enforce every applicable AD-052 criterion separately, identify the two Canonical-only criteria as inapplicable, derive Route F without precedent reclassification, bind the exact `0.3.2` reviewed body and all nine Accepted consumers, and synchronize the current status/consumer readers through AD-054 while preserving the AD-053 predecessors. They preserve Q1/Q2/Q6/Q7/Q8/Q10/Q11/Q12 and reject any partial status/version/package change or loss of an activation/candidate/cycle/Concept non-implication.
- Assignment document-canonicalization probes re-evaluate every Canonical criterion on the live `ASSIGNMENT_T6` prefix, enforce `1.0.0 / Canonical` together with `DOCUMENT_PROMOTION=completed`, and reject any partial rollback, Concept-status movement, third-step completion, withheld-question closure or unmet-need satisfaction. They also derive OCP-006's L2 pass without permitting any OCP-006 lifecycle effect and verify all pre-transition carriers byte-for-byte.
- Assignment document-acceptance probes enforce every applicable AD-052 criterion separately, identify the Canonical-only criteria as inapplicable, derive Route F from OCP-005 rather than precedent, bind the exact `0.3.0` reviewed body and all seven Accepted consumers, keep the current OCP-023 completeness need unmet, and preserve exact pre-transition versions of every changed projection. They reject snapshot loss/substitution, partial status/version changes, any criterion-basis or non-implication mutation, and any return of a live scanner to the pre-transition state.
- Assignment Canonical-readiness probes apply all seven fixed AD-052 criteria without reusing Accepted-only evidence, rederive the five Canonical direct dependencies and two Canonical Concept dependencies, execute ten current Assignment witnesses and retain the same-slot T6 proof through the now-live exact `ASSIGNMENT_T6` selection prefix. The unmet OCP-023 need, eight withheld questions, `0.4.0 / Accepted` and Assignment `Accepted` remain independent mutation-live boundaries; the pre-selection `active_cycle_id: null` assertion is preserved in its exact predecessor.
- Assignment promotion-selection probes exhaust OCP-005/OCP-006/OCP-010 from live gate and document state, require OCP-005 to be the sole eligible candidate, exact-bind `ASSIGNMENT_T6` with only the Board-selection step completed, and reject either pending step completing by implication. They enforce all fifteen live cycle carriers, seventeen readers, sixteen byte-exact predecessor successions, full baseline blob/path/state/SHA-256 anchors, every document/Concept lifecycle non-change, and both ordered forward closure and reviewed atomic rollback to the valid pre-selection gate.
- Assignment Q2 amendment-attempt probes keep the question and `AMENDMENT_MODEL_ABSENT` blocker open, derive all seven current Accepted consumers, bind four absent owner obligations and prove with unchanged-history variants that current validation accepts changed established role/applicability values without supersession or amendment provenance; the evidence itself remains non-positive while the hypothetical closure is G4-gated.
...
- Assignment consumer-compatibility evidence derives all seven current Accepted `Depends-On: OCP-005` documents, pins each consumed text token to the bounded surface, replays four negative-exclusion and three positive-derivation fixtures, including OCP-006's moving-surface independence control, rejects any restored compatibility blocker or changed consumer result, and leaves all moving surfaces, three semantic blockers and promotion gates unchanged.
