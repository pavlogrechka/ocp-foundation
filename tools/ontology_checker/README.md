...
- Constraint document-status readiness probes derive seven lifecycle criteria from exact current OCP-001/OCP-016 tokens, reject promotion criteria invented by stable-surface practice, scan all 24 Accepted/Canonical primary OCP documents and bind seven current open-question carriers plus the complete 84-line unresolved/open lexical surface. They preserve AD-052's Draft subject on its baseline while the live sweep includes current Accepted OCP-006; Canonical L2 still fails on Draft OCP-005.
- Constraint document-acceptance probes enforce every applicable AD-052 criterion separately, identify the two Canonical-only criteria as inapplicable, derive Route F without precedent reclassification, bind the exact `0.3.2` reviewed body and all nine Accepted consumers, synchronize all six live status/consumer readers, and prove with a discriminating fixture that OCP-006 consumes no moving Assignment surface. They preserve Q1/Q2/Q6/Q7/Q8/Q10/Q11/Q12 and reject any partial status/version/package change or loss of an activation/candidate/cycle/Concept non-implication.
