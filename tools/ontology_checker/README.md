...
`STATUS_PEER_VIEW_MISMATCH` additionally checks the current human-readable peer-table shape: inside a defining document section titled `Concept Status and Dependencies`, a table beginning with `Concept | Status` must render every registered Concept exactly once with the value from OCP-000. The rule scans all defining documents rather than a fixed path list. It ignores unregistered descriptive terms and does not cover historical tables outside that section or ASCII `[Status]` tree labels. OCP-000 remains the sole status authority; this check only detects drift and cannot grant, select or infer lifecycle.
