...
- Constraint document-status readiness probes derive seven lifecycle criteria from exact current OCP-001/OCP-016 tokens, reject promotion criteria invented by stable-surface practice, scan all 23 Accepted/Canonical primary OCP documents and bind six current open-question carriers plus the complete 81-line unresolved/open lexical surface. They assess every criterion separately: an OCP-006 Accepted transition remains possible only through a separate complete act, while Canonical L2 fails on Draft OCP-005; no status, question, candidate or cycle changes.
- Constraint document-acceptance probes enforce every applicable AD-052 criterion separately, identify the two Canonical-only criteria as inapplicable, derive Route F without precedent reclassification, bind the exact `0.3.2` reviewed body and all nine Accepted consumers, preserve Q1/Q2/Q6/Q7/Q8/Q10/Q11/Q12, and reject any partial status/version/package change or loss of an activation/candidate/cycle/Concept non-implication.
