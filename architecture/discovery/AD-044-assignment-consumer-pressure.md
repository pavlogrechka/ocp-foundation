...
3. immutability admits no post-Establishment value rewrite, so the established record is stable under the current bindings. This is an analytic consequence of the resolution definition, not a live before/after observation; the witness marks it `evidence_mode: analytic` rather than manufacturing a change that the resolution forbids.
...
Whole-Resource-only and explicit part scope on an Assignment keep `resource_ref=R-001`, so the current Resource/time/snapshot bindings can carry a whole-Resource occupancy query. Part-as-Resource identity does not: a synthetic effective Assignment bound to `R-001-PART-A` is rejected from an `R-001` snapshot as cross-bound, while its field-identical twin with only that Assignment reference restored to `R-001` is valid and yields `occupied=true` with `A-001` as witness. Rejections caused by a corrupt rule reference, a duplicate Assignment identity or an unresolvable snapshot reference also yield indeterminate, but their correspondingly whole-bound twins remain invalid and therefore do not establish part–whole closure.
...
The production governance validator replays nine observed resolution values through the committed `derive_resource_occupancy` implementation and existing synthetic fixtures: Q2 in-place versus superseding, Q3/Q9 observation cut and the paired Q5 part-bound/whole-bound control. It requires that observed map to equal exactly the nine declarations marked `evidence_mode: observed`. `POST_ESTABLISHMENT_IMMUTABILITY` is the sole `analytic` value because its defining prohibition leaves no legitimate changed post-state to observe. The focused test asserts both that boundary and causal binding. The Q5 regression attacks corrupt the rule reference, duplicate the Assignment identity and break the snapshot reference; none can produce the closure verdict because its whole-bound twin is invalid too. The named test `test_every_defensive_value_is_individually_fixture_and_mutation_live` covers every declared probe field, forbidden field/outcome, blocker/question mapping, resolution value/detail, evidence mode, adequacy value, blocker classification/summary, reason, gate/criterion value and scalar binding individually. Separate mutations prove blocker, current-need, probe and promotion-gate drift.
