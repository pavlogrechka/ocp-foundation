...
Six new unit-test methods provide individual rather than sampled evidence. For every one of the nine entries, deleting the snapshot fails, renaming the same bytes to `reviewed-contract-v9.9.9.md` fails, changing its bytes fails, removing its primary link fails and changing its current status fails. Removing each of the eight current-Accepted mappings separately fails coverage. The exact nine identity/status/version/digest/basis tuples and the exact retained set are also asserted, so removing or silently rewriting a declared element cannot leave the suite green.
