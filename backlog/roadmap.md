...
8. AD-016AC records the Board's exact OCP-010/Event selection and its compatibility/migration/rollback contract without promotion; the live gate still requires a separately mandated `EVENT_LIFECYCLE_PROMOTION_ACT` before any T6 status change. Define `Review-After` through its separately mandated normative governance act before any YR repair.
