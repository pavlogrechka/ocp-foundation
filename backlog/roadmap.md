...
| **Загальна foundation-готовність** | **≈70%** | Третій T4 micro-wave встановлює OCP-003/Resource `1.0.0 / Canonical`; OCP-007 Q2 remediation лишається `0.4.0 / Draft`, а AD-016P обирає O0 hold після виявлення live eighth lifecycle projection поза exact seven-file O7C unit |
...
1. Keep O0 hold: do not prepare an OCP-007 lifecycle or topology proposal while `tools/ontology_checker/README.md` remains an unclassified live eighth lifecycle projection outside the exact seven-file O7C unit.
2. If a fresh Board act proposes leaving O0, repeat the complete current-projection sweep, classify the checker-guide line explicitly, compare a newly named exact unit fairly, and require Fable review, Codex adjudication, green CI and explicit Pavlo/Architecture Board authorization on one unchanged head.
