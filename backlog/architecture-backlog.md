...
| AB-018 | Conflict між Operation як факт або похідний результат | Open | AD-023 на `main@f69e4b31` підтверджує закриті OCP-019 §9 / OCP-016 G4 ворота; AD-024 на `main@c8cf29d1` перевіряє запропонований OCP-004 consumer-profile шлях першим independence gate і не знаходить concrete current consumer, protected handling decision або legitimate non-Board owner/evaluator. Будь-яка повторна спроба має спершу незалежно довести ці елементи, потім окремо підготувати й прийняти profile до нового positive-model act |
...
| AB-025 | Reservation / Allocation як окремий Concept | Open | OCP-020 `0.2.0 / Accepted` знімає input-status prerequisite для partial/quantitative reservation, але не постачає Accepted capacity-result consumer, exact positive rule, legitimate owner/evaluator або object form; whole-Resource exclusivity лишається окремою розвилкою, жоден Concept чи record не вибрано |
...
| AB-037 | Quantity, demand and capacity model | Resolved | AD-025 обирає QC, а OCP-020 `0.2.0 / Accepted` приймає exact profile/unit/snapshot inputs та exact-unit aggregation без capacity-sufficiency або availability result; positive capacity activation потребує окремого акта та Accepted consumer за OCP-016 G4 |
| AB-038 | Conflict derivation model | Resolved | AD-022 обирає H0-B, а OCP-019 `0.1.0 / Draft` виконує Route C negative establishment boundary: одна чи кілька violations без exact Accepted consumer і legitimate criterion owner не створюють authoritative Conflict; positive model потребує окремого reopening за OCP-016 G4, AB-005/AB-018/AB-036 лишаються Open, а quantitative capacity activation окремо gated після AD-025/OCP-020 |
...
| AB-061 | Core Boundary admission and extension contract | Resolved | OCP-016 `1.0.0 / Canonical` реалізує AD-015B C3: стабільні Routes F/C/E/D/I, orthogonal Pattern form verdict, OCP-001 trigger/routing split, fail-safe route movement і no-projection stop rule без admission registry, score або P-002 |
| AB-062 | Foundation canonicalization readiness | Planned | AD-016Z незалежно реплеїть Assignment/Event як exact L2-admissible frontier, лишає Constraint blocked, резолвить positive target 14 через corrected repository-wide `Review-After` inventory 30 = 17 fulfilled / 13 open і обирає Y10D лише як scope окремо мандатованого Event discovery; поля, AB-015/016/017/018/019/020/023/028, T6–T10 і readiness ≈72% незмінні |
| AB-063 | Objective editorial-correction authority | Resolved | OCP-008 `0.3.0` реалізує AD-017B A+D: immutable stored statement, display exclusion, duplicate-ID rejection і exact historical consumer replay без amendment/revision authority або promotion |
