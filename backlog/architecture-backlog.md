...
| AB-061 | Core Boundary admission and extension contract | Resolved | OCP-016 `0.2.0 / Accepted` реалізує AD-015B C3: Routes F/C/E/D/I, orthogonal Pattern form verdict, OCP-001 trigger/routing split, fail-safe route movement і no-projection stop rule без admission registry, score або P-002 |
| AB-062 | Foundation canonicalization readiness | Discovery | AD-016A `0.2.0` порівнює A–F, перевіряє strict Canonical-or-same-act dependency floor і T0–T10 topology; `F → C` є leading hypothesis, але strategy selection лишається окремим AD-016B без status або `1.0.0` promotion |
