...
- AD-044 `0.1.0 / Discovery` перебирає всі три Assignment whole-freeze blockers і всі 10 resolution classes проти live OCP-023 completeness need: need signature зберігається скрізь, але без external completeness authority задоволення кожної розв'язки `undecidable-from-inside`, тому жодна не обирається й блокери не рухаються;
