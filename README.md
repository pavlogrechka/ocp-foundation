...
- AD-045 `0.1.0 / Discovery` перебирає всі шість AD-044 survivors проти 25 current primary bodies через 64-hit vocabulary sweep: жоден клас не `incompatible`, `WHOLE_RESOURCE_ONLY` є `compatible`, решта п'ять `underdetermined`; Draft OCP-005 є прямим provisional source без lifecycle promotion, а питання, blockers і authority не рухаються;
